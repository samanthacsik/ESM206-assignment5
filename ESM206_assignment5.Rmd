...
# Table 2: Fields in which females received PhDs, by year. Values indicated are counts, with proportions (by field of study) in italics. Data source: ___.
...
# QUESTION: Is there a significant effect of year on the proportion of year on PhD type awarded to female graduate students?
...
Between 1985 and 2015, female PhD students have shown a growing interest in the physical sciences and engineering, as the percent of female students specializing in the physical sciences has increased 8.6% from 10.1% to 18.7% (cite stacked bar graph) and the percent of female students specializing in engineering has increased 18.2% from 3.5% to 21.7%. This is reflected in the growing proportion of female students in the total number of students in the physical sciences has doubled between 1985 (0.16) and 2015 (0.34) and quadrupled between 1985 (0.06) and 2015 (0.23) in engineering.
...
##############################
# data is not continuous (pre-calculated median salaries by sex and field), so instead run a parametric test comparing salaries between males and females for postdoc and employment positions
...
#### discipline, rank, sex, rank*sex ####
facsal_lm7 <- lm(salary ~ discipline + rank + sex + rank*sex, data = faculty_salary)
summary(facsal_lm7)
plot(facsal_lm7)

#### discipline, rank*sex ####
facsal_lm8 <- lm(salary ~ discipline + rank*sex, data = faculty_salary)
summary(facsal_lm8)
plot(facsal_lm8)

#### rank, sex, discipline, discipline*sex ####
facsal_lm9 <- lm(salary ~ rank + sex + discipline + discipline*sex, data = faculty_salary)
summary(facsal_lm9)
plot(facsal_lm9)
# lowest AIC value but doesn't make sense because a Male in an Applied discipline makes less

#### rank, sex, discipline, sex*rank ####
facsal_lm10 <- lm(salary ~ rank + sex + discipline + sex*rank, data = faculty_salary)
summary(facsal_lm10)
plot(facsal_lm10)

#### rank, sex, discipline, rank*discipline ####
facsal_lm11 <- lm(salary ~ rank + sex + discipline + rank*discipline, data = faculty_salary)
summary(facsal_lm11)
plot(facsal_lm11)
```
...
AIC_lm7 <- AIC(facsal_lm7)
AIC_lm7
# 9098.989

AIC_lm8 <- AIC(facsal_lm8)
AIC_lm8
# 9098.989

AIC_lm9 <- AIC(facsal_lm9)
AIC_lm9
# 9096.663

AIC_lm10 <- AIC(facsal_lm10)
AIC_lm10
# 9098.989

AIC_lm11 <- AIC(facsal_lm11)
AIC_lm11
# 9098.414
