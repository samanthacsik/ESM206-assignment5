...
plot(full_lm1)
vif(full_lm1)
# FINDINGS: years_since & years_service both have VIF > 4 (some correlation, think critically about possible overlaps); this in combination with "years_plot" make me think that we should drop one of these predictor variables--I'm inclined to keep years_service because I think it makes more sense that the longer you've been faculty, the higher your rank (see fac_by_dept_plot) which typically means the higher your salary; 

# lm 2 includes rank, discipline, sex, years_since (eliminate correlated predictor var: years_since)
facsal_lm2 <- lm(salary ~ rank + discipline + years_service + sex, data = faculty_salary)
summary(facsal_lm2) # R2 = 0.44, p < 0.001
plot(facsal_lm2)
