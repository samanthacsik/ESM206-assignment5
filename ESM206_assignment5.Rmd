...
female_phd_perc <- female_phd %>% 
  mutate(perc_1985  = round((`1985`/tot_1985) * 100, 1), perc_2000 = round((`2000`/tot_2000) * 100, 1), perc_2015 = round((`2015`/tot_2015) * 100, 1)) %>% 
...
female_phd_plot <- ggplot(female_phd_long, aes(x = year_long, y = percent, fill = field_long)) + 
  ...
  geom_text(aes(x = year_long, y = percent, label = paste0(percent,"%")), size = 3,
