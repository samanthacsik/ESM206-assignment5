...
*Objectives* (we should put this in intro this time):  
  The following report uses publicly available demographic data to evaluate changes in the types of advanced academic degrees sought and by whom, as well as the salaries of doctorate degree earners at different stages of their careers. We use these data to assess: 
  (1) trends in the gender distribution of doctoral students in the US from 1985-2015,
  (2) shifts in female enrollment in doctoral programs in the Education, Engineering, Humanities & Arts, and the Physical Sciences. We do this by asking 
    (i) how the proportion of female graduate students in these fields have changed, and   
    (ii) how female graduate student research interests have shifted between these four fields
...
**Data & Methods:**
  ...
  Using data provided by the National Science Foundation's (NSF) Survey of Earned Doctorates (SED) (cite), we assessed changes in female graduate student research interests across four fields of study--education, engineering, humanities & arts, and the physical & earth sciences--between 1985, 2000, and 2015. A chi-squared test was used to determine whether there was a significant association between year and the type of PhDs awarded to women to evalue a shift in research interests through time ($alpha$ = 0.05). ADD AN'S ANALYSIS HERE. 
  To determine if there are presently (2015) pay disparities between men and women with graduate degrees, we then compared salaries between sexes in both postdoc and non-postdoc employment positions using a Mann-Whitney U test ($alpha$ = 0.05). Data was again provided by NSF's SED.
...
fields_line <- ggplot(line_df, aes(x = year, y = female_prop, group = Field, color = Field)) +
  ...
  scale_color_manual(values = wes_palette(name = "GrandBudapest2", 4, type = c("discrete"))) +
  expand_limits(y = 1) +
  theme_bw() +
...
*Salary differences between male and female PhD recipients*
  A Mann-Whitney U reveals no significant difference between salaries made by male and female postdocs (W = `r postdoc_u$parameter`, *p* = `r round(postdoc_u$p.value, 2)`) with a only a negligible effect size (Cliff's delta = `r round(postdoc_d$estimate, 2)`). Despite overall greater salaries for PhD recipients (STATS HERE) in non-postdoc employment positions, the difference between salaries earned by males and females remains nonsignificant (W = `r employed_u$parameter`, *p* = `r round(employed_u$p.value, 2)`) with a small effect size (Cliff's delta = `r round(employed_u$estimate, 2)`).
